...
family_rules = [
    ...
    IF(AND('parent (?z) (?x)', 'parent (?z) (?y)', NOT('same-identity (?x) (?y)')),
       THEN('sibling (?x) (?y)')),
    IF(AND('sibling (?x) (?y)', 'male (?x)'), THEN('brother (?x) (?y)')),
    IF(AND('sibling (?x) (?y)', 'female (?x)'), THEN('sister (?x) (?y)')),
    ...
    IF(AND('parent (?a) (?x)', 'parent (?b) (?y)', 'sibling (?a) (?b)'),
       THEN('cousin (?x) (?y)')),
...
HOW_MANY_HOURS_THIS_PSET_TOOK = 'foo'
WHAT_I_FOUND_INTERESTING = 'foo'
WHAT_I_FOUND_BORING = 'foo'
